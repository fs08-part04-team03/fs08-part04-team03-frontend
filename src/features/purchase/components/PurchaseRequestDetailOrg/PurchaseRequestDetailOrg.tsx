...
const BudgetInfo = ({ monthlySpending, remainingBudget, budgetAfterPurchase }: BudgetInfoProps) => (
  <InfoSection title="예산 정보">
    <InfoRowSingle
      label="이번 달 지출액"
      value={<PriceText value={monthlySpending} className="text-14 tablet:text-16" />}
    />
    <InfoRowSingle
      label="이번 달 남은 예산"
      value={<PriceText value={remainingBudget} className="text-14 tablet:text-16" />}
    />
    <InfoRowSingle
      label="구매 후 예산"
      value={<PriceText value={budgetAfterPurchase} className="text-14 tablet:text-16" />}
    />
  </InfoSection>
);
